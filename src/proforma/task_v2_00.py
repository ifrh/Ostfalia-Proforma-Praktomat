...
class Task_2_00:
    ...
    def get_optional_xml_element_text(self, xmlTest, xpath):
        try:
    ...
    def get_required_xml_element_text(self, xmlTest, xpath, namespaces, msg):
        if xmlTest.xpath(xpath, namespaces=namespaces) is None:
    ...
    def set_visibilty(self, instance):
    ...
    def set_task_description(self):
        xml_description = self.xml_dict.get("description")
        if xml_description is None:
            self.new_task.description = "No description"
        else:
            self.new_task.description = xml_description
        #new_task.save()



    def set_task_title(self):
        xml_title = self.xml_dict.get("title")
        if xml_title is None:
            self.new_task.title = "No title"
        else:
            self.new_task.title = xml_title
        #new_task.save()
    ...
    def create_file_dict_func(self, xml_obj, external_file_dict=None, ):
        namespace = self.ns
    ...
    def set_test_base_parameters(self, inst, xmlTest):
        ...
        inst.test_description = self.get_optional_xml_element_text(xmlTest, "p:description")
    ...
    def create_java_unit_checker(self, xmlTest, val_order, test_file_dict):
        ...
        inst.class_name = self.get_required_xml_element_text(xmlTest,
        ...
        if xmlTest.xpath("p:test-configuration/unit:unittest[@framework='JUnit']", namespaces=checker_ns):
            junit_version = self.get_optional_xml_attribute_text(xmlTest,
    ...
    def import_task(self):
        ...
        self.new_task = Task.objects.create(title="test",
                                       description="",
        ...
        try:
            ...
            self.set_submission_restriction()
            ...
            if self.dict_zip_files is None:
                create_file_dict, test_file_dict, list_of_modelsolution_refs_path = \
                    self.create_file_dict_func(xml_obj=self.xml_obj)
            else:
                create_file_dict, test_file_dict, list_of_modelsolution_refs_path = \
    ...
    def getitem_from_dict(self, dataDict, mapList):
    ...
    def set_submission_restriction(self):
        path = ['submission-restrictions']
        max_size = None
        restriction = self.getitem_from_dict(self.xml_dict, path)
        ...
        # convert to KB
        self.new_task.max_file_size = int(max_size) / 1024
